...
const {extractLessons, extractTasks, extractCourseTitle, extractText, extractQuestion, extractSingleChoice, extractVideoInformation} = require('./alura-scrapper')
...
const handleTask = async ({task, indexTask, page, tasks, courseTitle, indexLesson, lesson}) => {
  ...
  if (await fs.exists(Path.join(...pathArr))) {
    console.log(
  ...
  switch (task.type) {
    case 'text':
      ...
      taskInfo = await extractText({page})
      break;
    case 'singleChoice':
      console.log(`\n[TASK] [${_colors.yellow(task.title)}] ${_colors.blue('Single choice found')}`)
      taskInfo = await extractSingleChoice({page})
      break;
    case 'openQuestion':
      console.log(`\n[TASK] [${_colors.yellow(task.title)}] ${_colors.blue('Question found')}`)
      taskInfo = await extractQuestion({page})
      break;
    case 'video':
      console.log(`\n[TASK] [${_colors.yellow(task.title)}] ${_colors.blue('Video found')}`)
      taskInfo = await extractVideoInformation({page})
  ...
  await fs.writeJson(`${directoryToSave}/${taskName}.json`, taskInfo);
...
for (var i = 0; i < courserlist.length; i++) {
  var curse = `https://cursos.alura.com.br/course/${courserlist[i++].slug}`
  downloadCourser(curse)
}
